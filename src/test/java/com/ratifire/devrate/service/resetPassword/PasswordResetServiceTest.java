...
public class PasswordResetServiceTest {
    ...
    void resetPassword_WithValidCode_UpdatesPassword() {
        ...
        UserSecurity userSecurity = new UserSecurity(); // Создаем экземпляр UserSecurity

        when(emailConfirmationUUIDService.findUUIDCode(code)).thenReturn(emailConfirmationCode);
        when(userService.getById(user.getId())).thenReturn(user);
        when(userSecurityService.findByUserId(user.getId())).thenReturn(userSecurity); // Изменено на возврат userSecurity напрямую
